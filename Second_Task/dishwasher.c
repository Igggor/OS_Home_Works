...
/* --- FIFO --- */
char fifo_path[] = "/tmp/dish_fifo_XXXXXX";
int fifo_fd_w = -1, fifo_fd_r = -1;

int ipc_fifo_init(int limit) {
    (void)limit;
    char path[256];
    snprintf(path,sizeof(path),"/tmp/dish_fifo_%d",getpid());
    unlink(path);
...
int ipc_shm_init(int cap) {
    ...
    unsigned short vals[3]; vals[0]=1; vals[1]=cap; vals[2]=0;
...
struct sembuf P_op = {0,-1,0}, V_op = {0,1,0};


int sem_op(int semnum, short op) {
    struct sembuf s;
    s.sem_num = semnum; 
    s.sem_op = op; 
    s.sem_flg = 0;
...
int ipc_filesem_init(int cap) {
    ...
    snprintf(filebuf_path, sizeof(filebuf_path), "/tmp/dish_buf_%d.bin", getpid());
    ...
    struct {int in; int out; int cap;} hdr;
    hdr.in=0; hdr.out=0; hdr.cap=cap;
    ...
    char name[64];
    snprintf(name,sizeof(name),"/dish_mutex_%d", getpid());
    sem_unlink(name);

    file_sem_mutex = sem_open(name, O_CREAT|O_EXCL, 0600, 1);
    snprintf(name,sizeof(name),"/dish_empty_%d", getpid());

    sem_unlink(name);
    file_sem_empty = sem_open(name, O_CREAT|O_EXCL, 0600, cap);
    snprintf(name,sizeof(name),"/dish_full_%d", getpid());
...
int ipc_filesem_cleanup() {
    ...
    char name[64];
    snprintf(name,sizeof(name),"/dish_mutex_%d", getpid()); sem_unlink(name);
    snprintf(name,sizeof(name),"/dish_empty_%d", getpid()); sem_unlink(name);
    snprintf(name,sizeof(name),"/dish_full_%d", getpid()); sem_unlink(name);
    unlink(filebuf_path);
    return 0;
}
...
int ipc_init(int mode, int cap) {
    switch(mode) {
        case MODE_PIPE: return ipc_pipe_init(cap);
        case MODE_FIFO: return ipc_fifo_init(cap);
        case MODE_MSG: return ipc_msg_init(cap);
        case MODE_SHM: return ipc_shm_init(cap);
        case MODE_SOCK: return ipc_sock_init(cap);
        case MODE_FILESEM: return ipc_filesem_init(cap);
        default: return -1;
    }
}
...
int ipc_cleanup(int mode) {
    switch(mode) {
        case MODE_PIPE: return ipc_pipe_cleanup();
        case MODE_FIFO: return ipc_fifo_cleanup();
        case MODE_MSG: return ipc_msg_cleanup();
        case MODE_SHM: return ipc_shm_cleanup();
        case MODE_SOCK: return ipc_sock_cleanup();
        case MODE_FILESEM: return ipc_filesem_cleanup();
        default: return -1;
    }
}
...
int ipc_put(int mode, const char *s) {
    switch(mode) {
        case MODE_PIPE: return ipc_pipe_put(s);
        case MODE_FIFO: return ipc_fifo_put(s);
        case MODE_MSG: return ipc_msg_put(s);
        case MODE_SHM: return ipc_shm_put(s);
        case MODE_SOCK: return ipc_sock_put(s);
        case MODE_FILESEM: return ipc_filesem_put(s);
        default: return -1;
    }
}
...
int ipc_get(int mode, char *out) {
    switch(mode) {
        case MODE_PIPE: return ipc_pipe_get(out);
        case MODE_FIFO: return ipc_fifo_get(out);
        case MODE_MSG: return ipc_msg_get(out);
        case MODE_SHM: return ipc_shm_get(out);
        case MODE_SOCK: return ipc_sock_get(out);
        case MODE_FILESEM: return ipc_filesem_get(out);
        default: return -1;
    }
}
